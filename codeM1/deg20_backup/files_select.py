...
dir_save=r"E:\Liu\20240405\diff_img_raw\beta_20deg\selected\h76_w113\\"
os.makedirs(dir_save,exist_ok=True)

imgs=glob.glob(r"E:\Liu\20240405\diff_img_raw\beta_20deg\*.tif") #生の回折図形のディレクトリを指定して、関数globを使用してすべての回折図形のディレクトリを取得しておく
path=r"E:\Liu\20240405\diff_img_raw\beta_20deg\\" #上と同じディレクトリを指定
files=os.listdir(path) #回折図形の元々のファイル名を取得しておく（ファイル名を変更しない）

w_defo=322 #回折図形を取得した全体の視野のwを入力

y_offset=108 #粒界抽出領域のy座標のスタート座標を指定する
h=76 #粒界抽出領域のy方向の幅（高さ）を指定する
x_offset=180 #xも上記と同様に指定する
w=113
