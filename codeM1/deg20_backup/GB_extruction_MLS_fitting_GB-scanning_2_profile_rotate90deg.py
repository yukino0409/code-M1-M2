...
#読み込み画像を90°回転させることでフィッティング方向を90°回している

dir_save=r"E:\Liu\20240226_stereo_with_sato\piecewise_regression\20deg\rotate_90deg\\"
...
#時計回り
def rotate_90deg_CW(array):
    array=(np.fliplr(np.transpose(array,(1,0)))).astype(np.uint8)
    return array
#反時計回り
def rotate_90deg_CCW(array):
    array=(np.flipud(np.transpose(array,(1,0)))).astype(np.uint8)
    return array

#ナンバリング画像を読み込む
numbering=cv2.imread(r"E:\Liu\20240226_stereo_with_sato\MLS\numbering_map\20deg\numbering.tif",0)
numbering=rotate_90deg_CW(numbering)
#ASTARの方位マッピングデータから作成したマスク画像のディレクトリ
masks=glob.glob(r"E:\Liu\20240226_stereo_with_sato\mask_imgs\20deg\opt\*.tif")
#隣り合う粒のコンビネーションを出力したcsvファイルを読み込む
with open(r"E:\Liu\20240226_stereo_with_sato\MLS\grain_combination_20deg.csv") as f:
    reader=csv.reader(f)
    data=list(reader)
    data=np.array(data)
    combination=data.astype(float).astype(int)
#MLS結果(マッピング)のディレクトリ
path=r"E:\Liu\20240226_stereo_with_sato\MLS\mapping_GB-numbering\20deg\\"
#ASTARの粒界ピクセルを番号付けして分離した画像を読み込む
path_GB_numberings=r"E:\Liu\20240226_stereo_with_sato\MLS\numbering_map\20deg\GB_numbering\\"
...
array_GB_center_for_save=rotate_90deg_CCW(array_GB_center)
cv2.imwrite(dir_save+"GB_center.tif",array_GB_center_for_save)
...
for num_base_1,num_base_2 in combination:
    ...
    mask_base1=rotate_90deg_CW(mask_base1)
    mask_base2=rotate_90deg_CW(mask_base2)
    ...
    GB_numbering=rotate_90deg_CW(GB_numbering)
    ...
    mask_for_save=rotate_90deg_CCW(mask)
    name_mask=str(num_base_1).zfill(3)+"_"+str(num_base_2).zfill(3)+".tif"
    cv2.imwrite(dir_save_mask+name_mask,mask_for_save)
    ...
    img_MLS=rotate_90deg_CW(img_MLS)
    ...
    result_array_each_for_save=rotate_90deg_CCW(result_array_each)
    result_array_each_fill_for_save=rotate_90deg_CCW(result_array_each_fill)
    name=str(num_base_1).zfill(3)+"_"+str(num_base_2).zfill(3)+"_ref"+str(left_num).zfill(3)+".tif"
    cv2.imwrite(dir_save_each+name,result_array_each_for_save)
    cv2.imwrite(dir_save_fill+name,result_array_each_fill_for_save)
...
result_array_for_save=rotate_90deg_CCW(result_array)
result_array_2_for_save=rotate_90deg_CCW(result_array_2)
result_array_bin_for_save=rotate_90deg_CCW(result_array_bin)
cv2.imwrite(dir_save+"result.tif",result_array_for_save)
cv2.imwrite(dir_save+"result_both_max-val.tif",result_array_2_for_save)
cv2.imwrite(dir_save+"result_fill.tif",result_array_bin_for_save)
