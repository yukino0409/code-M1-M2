...
#マッピング結果を保存するディレクトリ
dir_save=r"E:\Liu\20240226_stereo_with_sato\MLS\mapping_GB-numbering\20deg\\"
os.makedirs(dir_save,exist_ok=True)
#ナンバリングした画像を保存するディレクトリ
dir_save_num=r"E:\Liu\20240226_stereo_with_sato\MLS\numbering_map\20deg\\"
os.makedirs(dir_save_num,exist_ok=True)

#回折図形(生データ,tif)のディレクトリ
imgs=glob.glob(r"E:\Liu\20240226_stereo_with_sato\diff_img_raw\20deg\selected\h166_w180\*.tif")
...
bases=glob.glob(r"E:\Liu\20240226_stereo_with_sato\MLS\base_diff_imgs\20deg\*.tif")
...
def numbering_GB(array,kernel_size): #ナンバリング画像をインプット
    ...
    for y,x in index:
        ...
        array_target=array_target[~(array_target==0)]

        multimode=statistics.multimode(array_target) #複数の最頻値が出る場合に対応した関数（リストで返される）
        if len(multimode)==1:
            array_target=array_target[~(array_target==multimode[0])]
            mode_second=statistics.mode(array_target) #2番目の最頻値を求める場合は1つだけ最頻値を抽出する
